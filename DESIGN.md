...
Every number in this document was measured against the running app, not estimated.
Contrast ratios are WCAG 2.1 relative luminance. Lightness and chroma figures are
CIELAB and OKLCH, never HSL, for reasons the Color section explains.
...
### The rule this system got wrong, and the correction
...
saw an order the arithmetic denied. Three separate places asserted the property
and none of them could catch it, because all three checked a representation
...
### Text ramp, with the semantic mapping
...
### The category chip: one neutral chip, form carries taxonomy
...
Five states that will be confused with each other unless each is named. Each is a
dashed chip with distinct label text:
...
| not applicable | `not applicable` | the question does not attach to this part |
| structurally unknowable | `cannot be established` | no source could answer this even in principle |

Distinct from all five, and rendered as a **solid** chip because it is an assertion
...
### Banned widgets, and what to use instead
